# -*- coding: utf-8 -*-
"""
Created on Wed Nov 13 15:02:31 2024
...
def brownian_surface(R=2,N=1000, H=0.95):
...
if __name__=="__main__":
    ...
    field = brownian_surface(R=Rval,N=400, H=0.85)
    X, Y = np.meshgrid(np.linspace(-Rval/2,Rval/2,field.shape[0]),
                       np.linspace(-Rval/2,Rval/2,field.shape[1]))
    ax.plot_surface(X, Y, field)
    # plt.imshow(field1)
    plt.title('Fractional Gaussian Field - H.85')
    #plt.show()
    plt.savefig("GaussianField.png",dpi = 1000)